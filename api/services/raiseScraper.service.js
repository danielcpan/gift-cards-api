...
const fetchData = async pageNumber => {
  ...
  axios
    .get(`${RAISE_URL}?page=${pageNumber}`)
    .then(response => {
      const $ = cheerio.load(response.data, { xml: { normalizeWhitespace: true } });

      const maxPage = Math.max(
        ...Array.from($('.pagination li a')).filter(el => !isNaN(parseInt(el.children[0].data)))
      );
      ...
      const giftCards = Array.from($('.product-source a')).map(el =>
        ...
        ({
          ...
          raiseId: el.attribs.href.substring(
            '/buy-'.length,
            el.attribs.href.length - '-gift-cards'.length
          ),
          brandName: JSON.parse(el.attribs['data-ga']).label,
          discount:
            Math.round(
              (parseFloat(JSON.parse(el.attribs['data-ga']).value) + Number.EPSILON) * 100
            ) / 100,
